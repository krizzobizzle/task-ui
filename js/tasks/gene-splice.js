/* =============================================
   ...
   4-phase escalating memory mini-game (Botanics)
   ============================================= */

const GeneSpliceTask = {
  // --- Configuration ---
  PHASE_COUNT: 4,
  SEGMENT_COUNTS: [3, 4, 5, 6],       // escalating strand lengths
  MAX_ATTEMPTS: 3,
  COOLDOWN_DURATION: 10,
  REVEAL_DURATIONS: [3000, 3000, 2500, 2000],
  CORRECT_FLASH_DURATION: 3000,        // show correct answer for 3s on fail

  GENE_NAMES: ['adenine', 'thymine', 'guanine', 'cytosine'],
  PHASE_LABELS: ['Chloroplast Strand', 'Root System Strand', 'Pollen Core Strand', 'Seed Vault Strand'],
  ...
  init() {
    ...
    // Bind palette buttons for all 4 phases
    for (let p = 0; p < this.PHASE_COUNT; p++) {
    ...
    // Generate all 4 patterns upfront
  ...
  _generateAllPatterns() {
    ...
    for (let p = 0; p < this.PHASE_COUNT; p++) {
      const pattern = [];
      const count = this.SEGMENT_COUNTS[p];
      for (let i = 0; i < count; i++) {
        const idx = Math.floor(Math.random() * this.GENE_NAMES.length);
        pattern.push(this.GENE_NAMES[idx]);
      }
  ...
  _startPhase(phaseIdx) {
    ...
    const segCount = this.SEGMENT_COUNTS[phaseIdx];
    this.playerInput = new Array(segCount).fill(null);

    // Build strand
    this._buildStrand(phaseIdx);
  ...
  _revealPattern(phaseIdx) {
    ...
    segments.forEach((seg, i) => {
      const colour = this.patterns[phaseIdx][i];
  ...
  // Input Handling
  // =========================================
  _onSegmentClick(segIdx) {
    if (this.inputLocked || this.isRevealing || this.cooldownActive) return;

    this._clearSelection();

    this.selectedSlot = segIdx;
    const seg = this._getSegmentEl(this.currentPhase, segIdx);
    if (seg) {
  ...
  _onPaletteClick(phaseIdx, colourName) {
    ...
    const segIdx = this.selectedSlot;

    // Store input
    this.playerInput[segIdx] = colourName;

    // Update segment visual
    const seg = this._getSegmentEl(phaseIdx, segIdx);
    if (seg) {
      this.GENE_NAMES.forEach(name => seg.classList.remove(`colour-${name}`));
  ...
  _clearSelection() {
    if (this.selectedSlot === null) return;
    const seg = this._getSegmentEl(this.currentPhase, this.selectedSlot);
    if (seg) {
  ...
  _autoSelectNextEmpty(phaseIdx) {
    ...
    for (let i = 0; i < segCount; i++) {
      if (this.playerInput[i] === null) {
        this.selectedSlot = i;
        const seg = this._getSegmentEl(phaseIdx, i);
  ...
  _allSlotsFilled() {
    return this.playerInput.every(v => v !== null);
  },

  _getSegmentEl(phaseIdx, segIdx) {
    const strandArea = document.getElementById(`strand-area-${phaseIdx + 1}`);
    if (!strandArea) return null;
    return strandArea.querySelector(`.segment[data-segment="${segIdx}"]`);
  },
  ...
  _checkAnswer(phaseIdx) {
    ...
    const pattern = this.patterns[phaseIdx];
    for (let i = 0; i < pattern.length; i++) {
      if (this.playerInput[i] !== pattern[i]) {
        allCorrect = false;
        wrongIndices.push(i);
      }
    }
    ...
    if (allCorrect) {
      this._onPhaseCorrect(phaseIdx);
    } else {
      this._onPhaseWrong(phaseIdx, wrongIndices);
    }
  },

  _onPhaseCorrect(phaseIdx) {
    const strandArea = document.getElementById(`strand-area-${phaseIdx + 1}`);
    ...
    if (phaseIdx < this.PHASE_COUNT - 1) {
      ...
      // Final phase complete — helix merge then completion
  ...
  _onPhaseWrong(phaseIdx, wrongIndices) {
    ...
    // Mark wrong segments with error style
    wrongIndices.forEach(idx => {
      const seg = strandArea.querySelector(`.segment[data-segment="${idx}"]`);
      if (seg) {
        seg.classList.add('error');
      }
    });

    // After a brief moment, flash the CORRECT answer for 3 seconds
    setTimeout(() => {
      // Clear player guesses and show correct pattern
      const segments = strandArea.querySelectorAll('.segment');
      segments.forEach((seg, i) => {
        this.GENE_NAMES.forEach(name => seg.classList.remove(`colour-${name}`));
        seg.classList.remove('error', 'empty', 'selected', 'pop');
        const colour = this.patterns[phaseIdx][i];
        seg.classList.add(`colour-${colour}`, 'revealing');
      });
      ...
      // Update instruction
      const instrEl = document.getElementById(`instruction-${phaseIdx + 1}`);
      if (instrEl) instrEl.textContent = 'Study the correct sequence...';

      // After 3s flash, restart the phase or trigger cooldown
      setTimeout(() => {
        if (this.attempts[phaseIdx] >= this.MAX_ATTEMPTS) {
          this._triggerCooldown(phaseIdx);
        } else {
          this._restartPhase(phaseIdx);
        }
      }, this.CORRECT_FLASH_DURATION);
    }, 800);
  },

  // Restart same phase (keeps attempt count)
  _restartPhase(phaseIdx) {
    this.inputLocked = true;
    this.isRevealing = true;
    this.selectedSlot = null;
  ...
  _endCooldown(phaseIdx) {
    ...
    // Full restart of this phase (resets attempts)
  ...
  // =========================================
  // Helix Merge Animation (final phase completion)
  // =========================================
